...
namespace ResoniteModLoader.JsonConverters
{
	...
	internal class EnumConverter : JsonConverter
	{
		...
		public override object ReadJson(JsonReader reader, Type objectType, object? existingValue, JsonSerializer serializer)
		{
			...
			{
				Logger.DebugFuncInternal(() => $"Deserializing a Elements.Core type: {objectType} from a {reader!.Value!.GetType()}");
				return deserialized!;
			}
			...
			throw new ArgumentException($"Could not deserialize a Elements.Core type: {objectType} from a {reader?.Value?.GetType()}. Expected underlying type was {underlyingType}");
		}

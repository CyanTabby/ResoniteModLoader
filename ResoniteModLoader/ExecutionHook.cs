...
#pragma warning disable CS0169
		...
		private static Type? __connectorType; // needed in all Resonite versions
		private static Type? __connectorTypes; // needed in Resonite 2021.10.17.1326 and later
#pragma warning restore CS0169
		...
		static ExecutionHook()
		{
			try
			{
				...
				DebugInfo.Log();
				ResoniteVersionReset.Initialize();
				HarmonyWorker.LoadModsAndHideModAssemblies(initialAssemblies);
				SplashChanger.SetCustom("Loaded");
			}
			catch (Exception e) // it's important that this doesn't send exceptions back to Resonite
			{

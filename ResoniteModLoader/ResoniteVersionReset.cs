...
{
	internal class ResoniteVersionReset
	{
		...
		internal static void Initialize()
		{
			...
			// attempt to map the PostX'd assemblies to Resonite's plugin list
		...
		private static HashSet<Assembly> GetExpectedPostXAssemblies()
		{
			List<Assembly?> list = new()
			{
				...
				Type.GetType("BusinessX.ResoniteClassroom, BusinessX")?.Assembly,
		...
		private static int? GetVanillaProtocolVersion()
		{
			...
			MethodInfo initializeShim = AccessTools.DeclaredMethod(typeof(Engine), "InitializeAssemblies");
			...
			{
				Logger.ErrorInternal("Could not find MoveNext method for Engine.InitializeAssemblies");
				return null;
			}
